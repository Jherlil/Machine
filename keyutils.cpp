...
bool load_puzzle_keys(const std::string& path) {
    std::ifstream f(path);
    if (!f.is_open()) {
        std::cerr << "[keyutils] Falha ao abrir arquivo de puzzle keys: " << path << std::endl;
        return false;
    }
    std::string line;
    size_t count = 0;
    ...
    std::cout << "[keyutils] " << count << " puzzle keys carregadas de " << path << std::endl;
    return true;
}
...
bool check_key(const char* priv_hex_c_str) {
    ...
    return puzzle_keys.find(priv_hex) != puzzle_keys.end();
}